...
class LlamaAI:
    ...
    def __init__(
        self, model_gguf_path: str, max_tokens: int, **llama_kwargs: Any
    ) -> None:
        """
        ...
        """
        self.model_path: str = model_gguf_path
        self.max_tokens: int = max_tokens
        self._max_input_tokens: Optional[int] = None
        self.llm: Optional[Llama] = None
        self.tokenizer: Optional[LlamaTokenizer] = None
        self._loaded: bool = False
        self._llama_kwrgs: Any = llama_kwargs
        self._embeddings_mode: bool = True
        self.load()
    ...
    def _try_fixing_format(
        self, text: str, only_letters: bool = False, rem_list_formatting: bool = False
    ) -> str:
        """
    ...
    def _check_loaded(self) -> None:
        ...
        if not self._loaded:
            try:
                ...
                raise Exception(
                    "Model not loaded! Please provide model settings when creating the class or use load_model method after creation."
                )
    ...
    def _set_input_token_limit(
        self, new_max_input_tokens: Optional[int] = None
    ) -> None:
        """
        ...
        """
        if new_max_input_tokens is None or (
            new_max_input_tokens is not None and new_max_input_tokens <= 0
        ):
            self._max_input_tokens = None
            print("Max input tokens limit cleared.")
        elif new_max_input_tokens < self.max_tokens:
            raise Exception(
                "The new maximum input tokens must be greater than the current maximum tokens."
            )
        elif (
            self._max_input_tokens is None
            or new_max_input_tokens != self._max_input_tokens
        ):
            self._max_input_tokens = new_max_input_tokens

    def set_max_tokens(
        self, new_max_tokens: int, max_input_tokens_limit: Optional[int] = None
    ) -> None:
        """
    ...
    def is_prompt_within_limit(self, text: str) -> bool:
        ...
        if self._max_input_tokens is not None:
            ...
            r = self.count_tokens(text) <= self.max_tokens
            print(
                f"Max input length not set, using max tokens: {self.max_tokens} tokens"
            )
        return r
    ...
    def postrocessed_generator(
        self,
        g: CreateCompletionResponse | Iterator[CreateCompletionStreamResponse],
        include_stop_str: str,
        stop_at_str: str,
    ) -> Generator[str, None, None]:
        for token in g:
            yield self._text_from_inference_obj(token)

        if include_stop_str:
            yield stop_at_str if stop_at_str is not None else ""
    ...
    def infer(
        ...
        if not self.is_prompt_within_limit(text):
            ...
            stop_at = (
                None if any([stop_at_str is None, stop_at_str == ""]) else stop_at_str
            )
            ...
            output = self.llm(
                text, stream=True, max_tokens=self.max_tokens, stop=stop_at
            )

            return self.postrocessed_generator(output, include_stop_str, stop_at_str)

    def _text_from_inference_obj(self, answer_dict: Any) -> str:
