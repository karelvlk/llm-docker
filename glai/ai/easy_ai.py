...
class EasyAI:
    ...
    def configure(
        ...
        if model_db_dir is None:
            print(
                f"Using provided verified models DB, files at {VERIFIED_MODELS_DB_DIR}"
            )
        ...
        if model_url is not None:
            ...
            or quantization_search is not None
            or keyword_search is not None
        ):
            self.find_model_data(
                name_search, quantization_search, keyword_search, search_only_downloaded
            )
        else:
            raise Exception(
                "Can't find model data. Please provide a model URL, GGUF file path, or model name/quantization/keyword."
            )

        self.load_ai(max_total_tokens)

    def load_model_db(
        self, db_dir: Optional[str] = None, copy_verified_models=True
    ) -> None:
        """
        ...
        self.model_db = ModelDB(
            model_db_dir=db_dir, copy_verified_models=copy_verified_models
        )

    def import_verified_models_to_db(
        self,
        model_name_quantization_list: Optional[list[Union[list | tuple]]] = None,
    ) -> None:
        """
    ...
    def find_model_data(
        ...
        if self.model_db is None:
            raise Exception("No model DB loaded. Use load_model_db() first.")
        model_data = self.model_db.find_model(
            model_name, quantization, keyword, only_downloaded
        )
        self.model_data = model_data
        return model_data

    def model_data_from_url(
        self,
        url: str,
    ...
    def model_data_from_file(
        ...
        if self.model_db is None:
            raise Exception("No model DB loaded. Use load_model_db() first.")
        model_data = ModelData.from_file(
            gguf_file_path,
            self.model_db.gguf_db_dir,
            user_tags,
            ai_tags,
    ...
    def _load_messages(self) -> None:
        ...
        if self.model_data is None:
            raise Exception(
                "No model data loaded. Use find_model_data(), get_model_data_from_url(), or get_model_data_from_file() first."
            )
        self.messages = AIMessages(
            user_tags=self.model_data.user_tags,
            ai_tags=self.model_data.ai_tags,
            system_tags=self.model_data.system_tags,
        )
    ...
    def load_ai(
        ...
        self.ai = LlamaAI(
            ...
            llama_kwargs={"chat_format": "llama2"},
        )
    ...
    def generate(
    ...
    ) -> Union[AIMessage, Generator[str, None, None]]:
        """
        ...
        if self.model_data is not None and self.model_data.has_system_tags():
            if system_message is not None:
                self.messages.set_system_message(system_message)
            else:
                print(
                    "WARNING: Model supports system messages, but no system message provided."
                )
        self.messages.add_user_message(user_message)
        print(f"Input to model: \n{self.messages.text()}")
        generated: str = ""
        if ai_message_tbc is not None:
            print("[DEBUG][ai_message_tbc]\n:", ai_message_tbc, "\n[/DEBUG]")
        ...
        if stop_at is None:
            stop_at = (
                ...
                if any(
                    [
                        self.messages.ai_tag_close is None,
                        self.messages.ai_tag_close == "",
                        self.messages.ai_tag_close != " ",
                    ]
                )
        ...
        g = self.ai.infer(
            ...
            only_string=True,
            stop_at_str=stop_at,
            include_stop_str=include_stop_str,
        )
        ...
        # if ai_message_tbc is not None:
        #     self.messages.edit_last_message(
        #         generated,
        #         self.model_data.get_ai_tag_open(),
    ...
    def count_tokens(
        self, user_message_text: str, ai_message_tbc: Optional[str] = None
    ) -> int:
        """
        ...
        if ai_message_tbc is not None:
            generation_messages.add_message(
                ai_message_tbc,
                self.messages.ai_tag_open if self.messages is not None else None,
                "",
            )

        return (
            self.ai.count_tokens(generation_messages.text())
            if self.ai is not None
            else 0
        )

    def is_within_context(
        self,
        prompt: str,
    ) -> bool:
        """
        ...
        """
        return self.ai.is_prompt_within_limit(prompt) if self.ai is not None else False
    ...
    def import_from_repo(
        ...
        user_tags: Tuple[str, str] = ("", ""),
        ai_tags: Tuple[str, str] = ("", ""),
        system_tags: Tuple[Optional[str], Optional[str]] = (None, None),
        keywords: Optional[str] = None,
        description: Optional[str] = None,
